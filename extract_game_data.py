...
logging.basicConfig(level=logging.INFO, filename='/home/belal/airflow/logs/data_extraction.log', filemode='a', format='%(name)s - %(levelname)s - %(message)s')
...
def main(start_index):
    with open("/home/belal/airflow/scripts/all_game_ids.json", "r") as f:
    ...
    if game_details:
        with open("/home/belal/airflow/scripts/game_details.json", "w") as f:
            json.dump(game_details, f, indent=4)
        logging.info(f"Successfully saved {len(game_details)} game details.")

    with open("/home/belal/airflow/scripts/processed_count.txt", "w") as f:
